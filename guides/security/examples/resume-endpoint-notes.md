...
// and contain bookmark metadata. The token remains valid until:
// 1. The bookmark is consumed (if AutoBurn = true)
// 2. The workflow instance is cancelled or deleted
// 3. Custom expiration logic is implemented (if needed)

// Note: There is no separate BookmarkResumeTokenLifetime configuration.
// To implement time-based expiration, consider:
// - Storing a timestamp in the bookmark payload
// - Validating expiration in your resume handler
// - Using scheduled cleanup of old bookmarks
```
