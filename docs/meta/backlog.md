...
- Before this run, the published GitBook is complete through `DOC-107`.
  `DOC-104`, `DOC-105`, and `DOC-108` remain available; `DOC-104` is selected
  as the recommended designer-facing slice.
- The latest released refs have advanced from `release/3.8.2` to
  `release/3.8.4`: Core `33181ae3048f628f591a0155b5665a8e4d1bcea2`, Studio
  `1c72dc02c837919059b60efe5df2ed57ff2db2d9`, and Extensions
  `e7d05ef930dfde2aa85bf9dcaf841e5a6ede0a1e`. The requested `release/3.8.0` is not
  the latest release branch for this run.
- DOC-104 StateMachine authoring and DOC-105 Studio activity-execution
  ...
  opportunities in current coverage are already tracked there. No additional
  distinct topic was found during this inventory; DOC-108 remains a separate
  workflow-definition lifecycle source-refresh slice.

### Current run plan (2026-09-20)

- Add or refresh a concise StateMachine authoring guide against Core, Studio,
  and Extensions `release/3.8.4`, covering states, initial/current state,
  trigger-driven and automatic transitions, conditions, entry/exit activities,
  transition actions, and completion behavior. Validate the Studio designer's
  actual editing affordances before documenting click steps.
- Present the topic for process designers and integrators: start with the
  runtime model, then show a small workflow example, authoring guidance, and
  lifecycle/troubleshooting notes. Keep persistence, hosting, and Studio
  claims within the release contracts actually verified.
...
### Current run selection (2026-09-20)

- Selected `DOC-104`: StateMachine authoring is a distinct designer/integrator
  gap. Existing activity reference content names the shape but does not explain
  how states and transitions execute, how to model entry/exit work, or where
  completion occurs.

### Current run completion (2026-09-20)

- Added `guides/state-machines.md`, linked it from `SUMMARY.md` and the
  activity reference, and documented the Core runtime lifecycle, C# and JSON
  shapes, automatic and triggered transitions, conditions, lifecycle slots,
  Studio's Diagram/Outline editor, validation, and troubleshooting against
  `release/3.8.4`.
...
- The authoritative release refs remain Core `8191ae305`, Studio `853952461`,
  ...
  `release/3.8.0`.
- Core source review found `Elsa.SasTokens` and the public
  `/events/trigger?t=...` route. The existing bookmark guide covers
  `/bookmarks/resume`, but does not cover the shared token service, event
  trigger URLs, or multi-node Data Protection key continuity.
- The selected slice is a security/operations guide that serves process
...
- Completed `DOC-098` by adding `guides/security/public-trigger-tokens.md`,
  ...
  alternative, token lifetime and replay boundaries, Data Protection key-ring
  continuity, conventional-host customization, and an operator checklist.
- Linked the guide from `SUMMARY.md`, Security & Hardening, and external
  application interaction; updated coverage metadata and closed the slice.
- Validated Core `release/3.8.0` at `8191ae305`, Studio at `853952461`, and
  Extensions at `66861ae082`. The requested branch did not change, and the
  guide is Core-backed; Studio does not add a separate token implementation,
  and the Extensions workbench only composes Core's `UseSasTokens` feature.
- Focused Core `Elsa.SasTokens.UnitTests` passed (`6/6`). Release-source
  assertions, repository-local links, SUMMARY targets, touched Markdown
  structure, source-link HTTP checks, and `git diff --check` also passed.
- Self-review pass 1 added direct links for the authenticated request model
  and token payloads. Delegated Luna xhigh review then required explicit
  expression-context typing, the authenticated route's asynchronous default,
  and a clearer host-versus-Elsa key-ring boundary; all three were fixed.
  Final pass 3 found no remaining high-priority factual, source-grounding,
  navigation, structure, regression, or formatting issues.
- Broad trailing-whitespace checking still reports one pre-existing issue in
  untouched `activities/blocking-and-triggers/README.md`; touched files are
  clean. Markdownlint, Vale, Lychee, and GitBook CLI are unavailable locally.
...
- The requested released branch remains `release/3.8.0`; these refs match the
  latest release worktrees used for this slice. The source branch name did not
  change.
...
### Current run completion (2026-08-29)

- Added `activities/file-storage.md`, linked it from `SUMMARY.md`, the
  activity reference, and the plugins/modules guide, updated current coverage,
  and completed DOC-085.
- Documented `UseFileStorage`, the default directory-backed provider, custom
  `IBlobStorage` selection, Open file and Save file input/output contracts,
  collection-to-ZIP behavior and its 3.8.0 finalization limitation,
  Studio/server boundaries, and operational safeguards.
- Release-source assertions, repository local-link and `SUMMARY.md` checks,
  ...
  passed. No local Markdownlint, Vale, Lychee, or GitBook command is available.
- No new follow-on slice was added after reviewing the file-storage module and
  its Core and Studio boundaries.
